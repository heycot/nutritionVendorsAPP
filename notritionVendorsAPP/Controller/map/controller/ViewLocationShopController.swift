...
//        configs()
    ...
    func addMap() {
        ...
        let camera = GMSCameraPosition.camera(withLatitude: (currentShop!.location?.latitude!)!, longitude: (currentShop!.location?.longitude!)!, zoom: zoomLevel)
    ...
    func setupMarkers() {
        let markers = listShop.map { (order) in
            return self.getMarker(order)
        }
        shopMarkers = markers
    }
